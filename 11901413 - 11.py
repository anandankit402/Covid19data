...
json_file = open('D:\\python\\coviddata.json','w')
json_file.write(str(url.text))
json_file.close()

#to load json file into variable
data = json.load(open('D:\\python\\coviddata.json'))


#writing into cvs file
output = csv.writer(open('D:\\python\\covid19.csv','w'))
output.writerow(data['raw_data'][0].keys())
for row in data['raw_data']:
    output.writerow(row.values())
    
#converting cvs file to excel
read_file = pd.read_csv (r'D:\\python\\covid19.csv',encoding="cp1252")
read_file.to_excel (r'D:\\python\\data.xlsx', index = None, header=True)

print('covid data is READY TO USE :')
print()

#read excel file and show it in graph
file=pd.read_excel('D:\\python\\data.xlsx')
...
pt.scatter(gender,value,color="green")
pt.grid(True)
pt.show()
